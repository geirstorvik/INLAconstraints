...
baseformula <- Y ~ offset(log(E)) + basis_tmin + basis_pdsi+ urban_basis1_pdsi + Vu+
  ...
  f(S1T2,model="generic0",Cmatrix = Q_st+diag(ns*nt)*eps,constr=F,
    extraconstr = list(A=as.matrix(PC2$A),e=rep(0,nrow(PC2$A))))#+f(S1T2_iid,model="iid")
...
baseformula.proj <- Y~ offset(log(E)) + basis_tmin + basis_pdsi+ urban_basis1_pdsi + Vu+
  ...
  f(S1T2,model="z",Z=as.matrix(PC2$P),precision=kap,Cmatrix=Q_st+diag(ns*nt)*eps,constr=F,
    extraconstr=list(A=as.matrix(PC2$A2),e=rep(0,nrow(PC2$A2))))#+f(S1T2_iid,model="iid")
...
if(run.knorr)
{
 dengue.proj.nb.knorr <- inla(baseformula.proj, family = "poisson",data =df,num.threads =4,inla.mode="experimental",control.fixed = list(
   prec.intercept =0.001),verbose=T,control.inla=list(cmin=0 ))
 saveRDS(dengue.proj.nb.knorr,"dengue.proj.nb.knorr.RDS")

 dengue.nb.knorr <- inla(baseformula, family = "poisson",data=df,num.threads = 4,control.fixed = list(
...
plotData=data.table::data.table(StandardModelE=dengue.nb.knorr$summary.random$S1T2$mean,
                                NewParametrizationE=dengue.proj.nb.knorr$summary.random$S1T2$mean[1:(ns*nt)],
                                StandardModelSD=dengue.nb.knorr$summary.random$S1T2$sd,
                                NewParametrizationSD=dengue.proj.nb.knorr$summary.random$S1T2$sd[1:(ns*nt)])

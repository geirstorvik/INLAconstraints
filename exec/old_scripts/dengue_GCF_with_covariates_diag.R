...
baseformula <- Y ~ offset(log(E)) + Vu + 
  ...
  f(T2,model="generic0",diagonal=eps,Cmatrix=Q_RW) +
  f(S1,model="generic0",diagonal=eps,Cmatrix = Q_ICAR,constr=T)+
  f(S1T2,model="generic0",diagonal=eps,Cmatrix = Q_st,constr=F,
...
#Plotting densities for tau_spatial
...
#Plotting densities for tau_interaction

...
library(data.table)
dataDir="~/Documents/SpatioTemporalModels/Package/SpaceTimePaper/inst/extdata"
...
NewConstraintsScaled=apply(NewConstraintsScaled,2,function(x){x[which(x>0)]=x[which(x>0)]-mean(x[which(x>0)])
                           return(x)})
NewConstraintsScaled=NewConstraintsScaled/sqrt(sum(NewConstraintsScaled[,1]^2))
...
resINLA2.full.Rue.Constr.Projector=inla(Y~f(main_temporal,model="generic0",Cmatrix=Q_RW2+diag(10)*1e-05,constr=T)+f(main_spatial,model="generic0",Cmatrix=Q_ICAR+diag(nrow(I2))*1e-05,constr=T)+f(interaction,model="z",precision=1e08,Z=Projector2,Cmatrix = Q_st+diag(ns*nt)*1e-05,constr=F,extraconstr = list(A=A_extra,e=rep(0,nrow(A_extra)))),data=data.sum.zero,verbose=T,family="poisson",control.fixed=prior.fixed,num.threads=4,control.predictor=list(compute=TRUE),inla.mode="experimental")
...
if(FALSE){
  resINLA2.full.Rue.Constr.Projector=inla(Y~f(main_spatial,model="generic0",Cmatrix=Q_ICAR+diag(nrow(I2))*1e-05,constr=T)+f(interaction,model="z",precision=1e08,Z=Projector2,Cmatrix = Q_st+diag(ns*nt)*1e-05,constr=FALSE),data=data.sum.zero,verbose=T,family="poisson",control.fixed=prior.fixed,num.threads=4,control.predictor=list(compute=TRUE),inla.mode="experimental")
  resINLA2.full.Rue.Constr.ordinary=inla(Y~f(main_spatial,model="generic0",Cmatrix=Q_ICAR+diag(nrow(I2))*1e-05,constr=T)+f(interaction,model="generic0",Cmatrix = Q_st+diag(ns*nt)*1e-05,constr=F,extraconstr = list(A=A,e=rep(0,nrow(A)))),data=data.sum.zero,verbose=T,family="poisson",control.fixed=prior.fixed,num.threads=4,control.predictor=list(compute=TRUE),inla.mode="experimental")
}
...
if(FALSE){
  ...
  A_extra_new=new_vectors$Q[,(nrow(I2)+ncol(NewConstraints)+-1+1):ncol(new_vectors$Q)]
  A_extra=cbind(t(A_extra_new),0*t(A_extra_new))
  A_extra=t(cbind(0*A_extra_new,A_extra_new))
}

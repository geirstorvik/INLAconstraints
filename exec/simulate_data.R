...
set.seed(20)
x_s_main= INLA::inla.qsample(Q=Q_ICAR+diag(ns)*1e-05,n=1,mu=rep(0,ns),constr=list(A=matrix(rep(1,ns),nrow=1),e=0))
x_t_main=INLA::inla.qsample(Q=Q_RW2+diag(nt)*1e-05,n=1,mu=rep(0,nt),constr=list(A=A_t,e=rep(0,2)))
x_st_main=INLA::inla.qsample(Q=Q_st+diag(nt*ns)*1e-05,n=1,mu=rep(0,nt*ns),constr=list(A=A_st,e=rep(0,nrow(A_st))))

#NB: we must add the slope to get a simple sum to zero constraint.
slope = rnorm(1,mean=0,sd=2)
slopes_st_spatial=rnorm(2*ns,mean=0,sd=3)
slopes_st_temp=rnorm(nt,mean=0,sd=3)
main=seq(1,ns*nt)%%ns
main[main==0]=ns
...
temporal_main_C=rep(seq(1,ns)/norm(seq(1,ns),"2"),ns)
...
for(i in 1:(nt)){
  print(i)
  mean_pred=mean_pred+slopes_st_temp[i]*temporal_C[i,]
}

for(i in 1:(ns)){
  print(i)
  mean_pred=mean_pred+slopes_st_spatial[i]*spatial_C[i,]
